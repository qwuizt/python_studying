...
def deposit(amount):
    ...
    if amount % DEPOSIT_INCREMENT != 0:
        print(f"Сумма пополнения должна быть кратной {DEPOSIT_INCREMENT} у.е.")
        return
...
def withdraw(amount):
    global balance, transactions, operations_count
    if amount % DEPOSIT_INCREMENT != 0:
        print(f"Сумма снятия должна быть кратной {DEPOSIT_INCREMENT} у.е.")
        return
